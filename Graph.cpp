...
DistVect * Graph::dijkstra(int src)
{
	...
	for (int count = 0; count < V; count++)
	{
		...
		for (int v = 0; v < V; v++)
		{
			...
			if (a[currentPathVertex][v] != -1 && dvptr->getMinDist(currentPathVertex) + a[currentPathVertex][v] < dvptr->getMinDist(v))
			{
...
int Graph::getNextClosestVertex(int v, int * prevPathVertex, DistVect* dvptr, int src)
{
	...
	for (int i = 0; i < V; i++)
	{
		if (a[v][i] != -1 && i != v && a[v][i] < minDistVertex && dvptr->isUnmarkedVertex(i))
		{
			minDistVertex = a[v][i];
			minDistVertexInd = i;
		}
